...
class OrderPreviewItem extends Component {
  constructor(props) {
    super(props)
    this.canvasRef = null
  }
  ...
  drawCanvas = () => {
    ...
    this.drawText(ctx, `ผู้รับ: ${order.reciver.name} T: ${order.reciver.phoneNumber}`, 20, 190, 14, 'bold')
    this.drawText(ctx, `${order.reciver.address}`, 20, 210, 14, 'bold')

    this.drawText(ctx, `ผู้ส่ง: ${order.sender.name} T: ${order.sender.phoneNumber}`, 20, 240, 13)
    this.drawText(ctx, `${order.sender.address}`, 20, 260, 13)

    this.drawText(ctx, `${order.date}, (${order.type}), ${order.weight}`, 20, 300, 13)

    this.drawText(ctx, `หมายเหตุ: ${order.eg}`, 20, 330, 13)
